...
void File64::_seekFile(__int64 i64NewPos) {
	...
	if (0xFFFFFFFF == (unsignet)SetFilePointer(hFile, (LONG)i64NewPos, &lHi, FILE_BEGIN))
...
void File64::_skipFile(__int64 bytes) {
	...
	if (0xFFFFFFFF == (unsignet)(lNewLow = SetFilePointer(hFile, (LONG)bytes, &lHi, FILE_CURRENT)))
		if ((dwError = GetLastError()) != NO_ERROR)
			throw MyWin32Error("File64: %%s", dwError);
